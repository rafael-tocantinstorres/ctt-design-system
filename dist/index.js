...
if (typeof window !== 'undefined' && window.customElements) {
  ...
  Promise.resolve().then(function () { return styles; }).catch(() => {
    // Fallback: try to load from dist/styles.css via link tag
    if (typeof document !== 'undefined') {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = new URL('./dist/styles.css', (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.js', document.baseURI).href))).href;
...
let CttButton, CttHeader, CttInputField, CttPage, CttRadioButton, CttTextareaFieldElement;
